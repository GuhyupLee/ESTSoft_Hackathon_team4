...
import os
import openai
from .utils import get_random_question, get_gpt_response, save_response, contains_question, save_user, load_users, verify_user, load_conversation, get_all_dates_in_month, summarize_responses, base_dir, save_guardian, load_guardian, verify_guardian
import random
import csv
...
def calendar_view(year, month):
    ...
    username = session.get('ward_username') if session.get('role') == 'guardian' else session['username']
    current_month_dates = get_all_dates_in_month(year, month)
    
    data_path = os.path.join(base_dir, 'app', 'data', 'responses.csv')
    if os.path.exists(data_path):
        df = pd.read_csv(data_path, encoding='utf-8-sig')
        recorded_dates = df[df['User'] == username]['Date'].unique().tolist()
    else:
        recorded_dates = []
